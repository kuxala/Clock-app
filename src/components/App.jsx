  ...
  useEffect(() => {
    ...
    const fetchTimeData = async () => {
      try {
        ...
        const response = await fetch(
          `http://worldtimeapi.org/api/timezone/${da.location.timezone}`
        );
        const data = await response.json();
    ...
    <div className={darkmode ? "wrapper-dark" : "wrapper"}>
      <div className="full-col">
        <div className="left-col">
          <animated.header
            className={dropDown ? "header-none" : "header-text"}
            style={fade}
          >
            <div>
              <p>{quote}</p>
      ...
      <animated.div
        className={dropDown ? "dropdown" : "dropdown-none"}
        style={dropDown ? fadeIn : fadeOut}
      >
